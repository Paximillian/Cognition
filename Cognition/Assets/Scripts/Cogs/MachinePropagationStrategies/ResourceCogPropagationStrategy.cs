...
public class ResourceCogPropagationStrategy : GeneralCogPropagationStrategy
{
    public override List<Tuple<BaseCog, BaseCog>> Propogate(NetworkPlayer i_Player, BaseCog i_RequestingCog, bool i_StopUnaffected = false)
    {
        ...
        {
            throw new ArgumentNullException("A resource cog can't be a driving cog (i.e. Have no cog requesting it to spin)");
        }
        else
        {
