...
////// Emissive:
                ...
                float3 node_544 = (lerp(_DamagedDiffuse_var.rgb,_BaseDiffuse_var.rgb,(1.0 - saturate((pow((_DamagePattern_var.rgb+_Damage),node_1300)*node_1300*_Damage))))*_Color.rgb); // Diffuse Color
                float3 node_1063 = (_GlowColor.rgb*_GlowPower);
                float3 emissive = saturate(((node_544*UNITY_LIGHTMODEL_AMBIENT.rgb)+node_1063));
                float3 LightDirection = normalize(float3(_LightPositionX,_LightPositionY,_LightPositionZ));
                float node_9923 = 8.0;
                float3 finalColor = emissive + ((node_544*pow(floor((0.2+max(0,dot(LightDirection,normalDirection))) * node_9923) / (node_9923 - 1),exp((_LightContrast*6.9+0.1))))*_LightColor.rgb*_LightDistance);
